...
architecture sim of tb_serial_to_parallel is
    ...
    function to_hex(slv : std_logic_vector(C_DATA_BITS - 1 downto 0)) return string is
        constant HEX_CHARS : string(1 to 16) := "0123456789ABCDEF";
    begin
        if is_x(slv) then
            return "XX";
        end if;
        return HEX_CHARS(to_integer(unsigned(slv(C_DATA_BITS - 1 downto C_DATA_BITS - 1))) + 1) &
               HEX_CHARS(to_integer(unsigned(slv(C_DATA_BITS - 2 downto 0))) + 1);
    end function;
